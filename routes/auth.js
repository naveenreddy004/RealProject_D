...
router.post('/register', upload.single('profilePhoto'), async (req, res) => {
  try {
    ...
    const existing = await Registration.findOne({ user: user._id, status: { $nin: ['rejected'] } });
    if (existing) {
      return res.status(400).json({ success: false, message: 'You already have an active registration. Please login to your portal.' });
    }
