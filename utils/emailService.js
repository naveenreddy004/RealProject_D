 ...
 *
 * Exactly TWO automated student emails:
 *   1. confirmation  — sent immediately upon registration success.
 *                      Subject: "Internship Registration Confirmed! - avRoN Tech"
 *   2. offerLetter   — sent after admin approves the payment, with the official
 *                      Offer Letter PDF attached.
 *                      Subject: "Official Internship Offer Letter & Onboarding Details - avRoN Tech"
 *
 * OTP emails still flow through `otp` (silent, transactional).
 */
...
// ── Common HTML scaffolding (clean, professional, no emoji noise) ─────────────
...
.tagline{font-size:12px;color:#5a6a7e;letter-spacing:0.06em;margin-top:6px;}
</style></head><body>
<div class="email-wrap">
  ...
  <div class="footer">
    <div>
      <a href="https://${DOMAIN}">News</a><span class="pipe">|</span>
...
async function sendConfirmationEmail(user, reg) {
  const html = wrap(`
    ...
    <p>Our team is currently reviewing your registration parameters. You will receive your official onboarding package and program credentials shortly with clear instructions on how to begin your technical journey.</p>

    <p class="muted">If you have any operational or setup questions, feel free to reach out to our active support help desk. Thank you for choosing ${BRAND} to power your career!</p>

    <hr class="divider">
    <div class="signoff">
      Best regards,<br>
      <b>The ${BRAND} Team</b><br>
      <a href="https://${DOMAIN}" style="color:#608BC1;text-decoration:none;">${DOMAIN}</a>
    </div>
  `);
...
// ── 2. Offer Letter (after admin approval) ────────────────────────────────────
async function sendOfferLetterEmail(user, reg, pdfBuffer) {
  const html = wrap(`
    <h2>Dear ${user.fullName},</h2>
    <p>We are pleased to offer you a <b>virtual internship position</b> at <b>${BRAND}</b>. Congratulations on your formal selection!</p>

    <p>Please find your official program tracking parameters detailed below:</p>
    <div class="kv">
      <div class="kv-row"><span class="lbl">Domain</span><span class="val">${reg.domain}</span></div>
    ...
    <p>Please review the attached formal Offer Letter document included with this message for your educational records. We highly encourage you to celebrate your placement, share your official selection letter, and tag our corporate engine on LinkedIn at <b>${DOMAIN}</b>.</p>

    <div class="steps">
      <div class="head">Steps to Complete Tasks and Access Certificates</div>
      <ol>
        <li>Log securely into your dashboard layout at <b>${DOMAIN}/dashboard</b>.</li>
        <li>Open the <b>"My Internships"</b> tab to view your active domain roadmaps.</li>
        <li>Review your weekly timelines, submit your task repositories, and link your verification videos directly through the interface modules.</li>
      </ol>
    </div>

    <p>If you have any questions, feel free to contact us at <a href="mailto:${SUPPORT_EMAIL}" style="color:#608BC1;">${SUPPORT_EMAIL}</a>. Welcome aboard!</p>

    <hr class="divider">
    <div class="signoff">
      Warm regards,<br>
      <b>Corporate Onboarding Team</b><br>
      ${BRAND} (<a href="https://${DOMAIN}" style="color:#608BC1;text-decoration:none;">${DOMAIN}</a>)
    </div>
  `);

  const attachments = [];
  if (pdfBuffer && pdfBuffer.length > 0) {
    // Attach directly from Buffer — no temp file needed
    attachments.push({
...
// ── 3. OTP (silent transactional) ─────────────────────────────────────────────
async function sendOTPEmail(user, otp) {
  const html = wrap(`
    <h2>Login verification code</h2>
    <p>Hello ${user.fullName}, use the code below to login to your <b>${BRAND}</b> dashboard. The code is valid for 10 minutes.</p>
    ...
    <p class="muted">If you didn't request this code you can safely ignore this email — no further action is required.</p>
  `);
...
async function sendPaymentRejectedEmail(user, reg, reason) {
  const html = wrap(`
    <h2>Dear ${user.fullName},</h2>
    <p>Unfortunately, we were <b>unable to verify your recent UPI payment</b> for the <b>${reg.domain}</b> internship program.</p>
    ...
    <p>Please log in to your portal and resubmit your payment with a valid UTR number and a clear screenshot of the transaction.</p>
    <div class="steps">
      <div class="head">What to do next</div>
      <ol>
        <li>Open your <b>avRoN Tech portal</b> at <a href="https://${DOMAIN}/portal" style="color:#608BC1;">${DOMAIN}/portal</a></li>
        <li>Go to the <b>Dashboard</b> tab and submit your payment again with the correct UTR.</li>
        <li>Make sure the screenshot clearly shows the transaction ID and amount.</li>
      </ol>
    </div>
    <p class="muted">If you believe this is a mistake, please contact us at <a href="mailto:${SUPPORT_EMAIL}" style="color:#608BC1;">${SUPPORT_EMAIL}</a> with your UTR and transaction screenshot.</p>
    <hr class="divider">
...
// Backwards-compat stubs — older code paths called these names. Each is now a
// no-op so we don't accidentally spam students with extra emails.
const noop = async () => {};
module.exports = {
